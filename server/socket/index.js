...
const initSocket = (server, corsOptions) => {
    ...
    io.on('connection', socket => {
        ...
        socket.on('SEND_MESSAGE', async (messageData) => {
            ...
            if (clientId) {
                // If type is room, receiver is roomId, check if room logic matches
                if (type === 'room') {
                    socket.to(receiver).emit('RECEIVE_MESSAGE', { ...messageData, unreadCount });
                } else {
        ...
        socket.on('ROOM_CREATED', ({ name, creator, invitedUser }) => {
            invitedUser.forEach(invitedUser => {
                ...
                if (socketId) {
                    socket.to(socketId).emit('INVITED_TO_ROOM', { message: `${creator} invited you to ${name}` });
                }
            });
        });

        socket.on('TOGGLE_PIN', ({ chatId, type, messageId, isPinned, senderId }) => {
            const receiverId = type === 'room' ? chatId : (chatId === senderId ? null : chatId); // Logic to find receiver
            if (type === 'room') {
                socket.to(chatId).emit('PIN_UPDATED', { messageId, isPinned });
            } else {
                const socketId = onlineUsers.find(({ userId }) => userId === chatId)?.socketId;
                if (socketId) {
                    socket.to(socketId).emit('PIN_UPDATED', { messageId, isPinned });
                }
            }
        });

        socket.on('SUMMARY_UPDATED', ({ chatId, type, summary, senderId }) => {
            if (type === 'room') {
                socket.to(chatId).emit('RECEIVE_SUMMARY', summary);
            } else {
                const socketId = onlineUsers.find(({ userId }) => userId === chatId)?.socketId;
                if (socketId) {
                    socket.to(socketId).emit('RECEIVE_SUMMARY', summary);
                }
            }
        });
    });
